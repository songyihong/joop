            ...
            function __childSelectors(selector, elements){
                ...
                for(var i = 0, length = elements.length; i < length; i++){
                    ...
                    for(var j = 0, len = childnodes.length; j < len; j++)
                        if(childnodes[j].nodeType == 1 && checkattr(childnodes[j], v.attrs) && (childnodes[j].tagName.toLowerCase() == v.tagName || v.tagName == '*')) tmpelements.push(childnodes[j]);
                }
                return tmpelements;
            }
...
var EventListener = Class({
    initialize : function(){
        EventListener.superclass.initialize.apply(this, arguments);
        this._Listeners = [];
    },
...
var AjaxResponse = Class({
    initialize : function(xmlHttp){
        AjaxResponse.superclass.initialize.apply(this, arguments);
...
/* class Ajax */
var Ajax = Class(EventListener, {
    initialize : function(){
        Ajax.superclass.initialize.apply(this, arguments);
    ...
    _onreadystatechange : function(runtime){
        ...
        if(runtime.xmlHttp.readyState == 4)
            runtime.xmlHttp.status == 200 ? runtime.notifyEvent("success", runtime.xmlHttp) : runtime.notifyEvent("failure", runtime.xmlHttp);
    }
});
